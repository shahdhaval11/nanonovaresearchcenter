        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {CORE_SERVICES.map((service, index) => (
            <Reveal key={service.title} delay={(index % 3) * 100}>
              <div className="group relative h-full overflow-hidden rounded-2xl border border-secondary-100 bg-white transition-all duration-300 hover:-translate-y-1.5 hover:border-transparent hover:shadow-xl">
                <div className="relative h-36 w-full overflow-hidden">
                  ...
                  />
                  <span
                    className={`absolute bottom-3 left-3 flex h-11 w-11 items-center justify-center rounded-xl bg-linear-to-br text-white shadow-md transition-transform duration-300 group-hover:scale-110 ${CARD_GRADIENTS[index % CARD_GRADIENTS.length]}`}
                  >
